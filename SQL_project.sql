--Creating a Database for the GoMyCode SQL Project
...
FOREIGN KEY (EMPLOYEE_int) REFERENCES Project_EMPLOYEES (EMPLOYEE_int);
...
WHERE PRODUCT_REF NOT IN (
    SELECT PRODUCT_REF FROM PRODUCTS
);
...
SELECT DISTINCT
    s.SUPPLIER_int AS Supplier_Number,
    s.COMPANY,
    s.ADDRESS,
    s.PHONE
FROM SUPPLIERS s
...
HAVING COUNT(DISTINCT od.PRODUCT_REF) <= 1;
